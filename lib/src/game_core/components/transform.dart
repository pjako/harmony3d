...
class TransformComponentSystem implements ComponentSystem {
  final String componentName = 'Transform';
  static final ComponentPool _componentPool = new ComponentPool(() => new Transform());
  ...
  Component createComponent() {
    return _componentPool.getFreeComponent();
  }
  ...
  const TransformComponentSystem();
  ...
  Map decodeComponents(List<Component> components, Map<String,dynamic> componentsData, Map<int, dynamic> objects) {
    ...
    var posBuffer = new Float32List.view( base64decoder.decode(componentsData['positions']) );
    var rotBuffer = new Float32List.view( base64decoder.decode(componentsData['rotations']) );
    var scaleBuffer = new Float32List.view( base64decoder.decode(componentsData['scales']) );
    for(int i=0; i<count; i++) {
      int p = i * 3;
      int r = i * 4;
      int s = i * 3;
      Transform transform = components[i];
      ...
      transform._localScale.storage[0] = scaleBuffer[s];
      transform._localScale.storage[1] = scaleBuffer[s+1];
      transform._localScale.storage[2] = scaleBuffer[s+2];
...
class Transform extends Component {
  ...
  @Serialize(SerializeType.vec3, customName: 'position')
  final Vector3 _localPosition;
  @Serialize(SerializeType.quat, customName: 'rotation')
  final Quaternion _localRotation;
  @Serialize(SerializeType.vec3, customName: 'scale')
  ...
  set position(Vector3 pos) {
    ...
    _transformFlag |= (dirty+posRotScaleDirty);
    if(parent != null) {
      //if(_transformFlag & 0 == 0) {
      //  _localPosition.setFrom(pos);
      //  return;
      //}
      Float32List parentMat = parent.transform._worldMatrix;
      print('${gameObject.name} : poschange');
      //final vec = _worldMat.transform3(new Vector3.zero());
  ...
  */
  ...
  void lookAt(Vector3 focusPoint, Vector3 upVector) {
    ...
    /*
    _transformFlag |= (dirty+posRotScaleDirty);
    setViewMatrix(_worldMat,position,focusPoint,upVector);
    quaternionFromMatrix2(_localRotation,_worldMat);
    _localRotation.normalize();
    */
    
    /*
    var upVector = new Vector3.zero();
    var rightVector = new Vector3.zero();
    var forwardVector = new Vector3.zero();
    var destMatrix = new Vector3.zero();
    
    Vector3 vec3 = _pool0;
    position.copyInto(vec3);
    vec3.sub(focusPoint);
    vec3.normalize();
    ...
    */
    ...
    _localRotation.normalize();
    print('focusPoint: ${focusPoint} localRot: ${_localRotation}');
  ...
  /*
  Vector3 get forward {
    double x = _localRotation.storage[0];
    double y = _localRotation.storage[1];
    double z = _localRotation.storage[2];
    double w = _localRotation.storage[3];
    //_updateWorldTransform();
    return new Vector3( 2.0 * (x * z + w * y),
                        2.0 * (y * x - w * x),
                  1.0 - 2.0 * (x * x + y * y));
  }*/

  //Matrix4 _getWorldTransform() => _updateWorldTransform();
  //Matrix4 get _internalWorldMatrix => _updateWorldTransform();

  //Matrix4 get _parentWorldTansform => gameObject.parent.transform._getWorldTransform();

  /*
  Matrix4 _updateWorldTransform() {
    if(_needsWorldMatrixUpdate == false) {
      return _worldTransform;
    }
    _updatePosRotScale();
    _updateWorldMatrix();
    _needsWorldMatrixUpdate = false;
    return _worldTransform;
  }*/
...
void worldMatrixFromPosRotScale(Float32List worldMatrix, Float32List localPosRotScale) {
  ...
  /*
  worldMatrix[0]  = sx * (1.0 - (yy + zz));
  worldMatrix[1]  = sy * (xy + wz);
  worldMatrix[2]  = sz * (xz - wy);

  worldMatrix[4]  = sx * (xy - wz);
  worldMatrix[5]  = sy * (1.0 - (xx + zz));
  worldMatrix[6]  = sz * (yz + wx);

  worldMatrix[8]  = sx * (xz + wy);
  worldMatrix[9]  = sy * (yz - wx);
  worldMatrix[10] = sz * (1.0 - (xx + yy));

  worldMatrix[12] = localPosRotScale[0];
  worldMatrix[13] = localPosRotScale[1];
  worldMatrix[14] = localPosRotScale[2];
  */
...
/*
 *   var pwm0  = parentWorldMat[0], pwm1 = parentWorldMat[1], pwm2 = parentWorldMat[2], pwm3 = parentWorldMat[3];
  outWorldMatrix[0] = pwm0*lm00 + pwm1*lm10 + pwm2*lm20 + pwm3*lm30;
  outWorldMatrix[1] = pwm0*lm01 + pwm1*lm11 + pwm2*lm21 + pwm3*lm31;
  outWorldMatrix[2] = pwm0*lm02 + pwm1*lm12 + pwm2*lm22 + pwm3*lm32;
  outWorldMatrix[3] = pwm0*lm03 + pwm1*lm13 + pwm2*lm23 + pwm3*lm33;

  pwm0 = parentWorldMat[4]; pwm1 = parentWorldMat[5]; pwm2 = parentWorldMat[6]; pwm3 = parentWorldMat[7];
  outWorldMatrix[4] = pwm0*lm00 + pwm1*lm10 + pwm2*lm20 + pwm3*lm30;
  outWorldMatrix[5] = pwm0*lm01 + pwm1*lm11 + pwm2*lm21 + pwm3*lm31;
  outWorldMatrix[6] = pwm0*lm02 + pwm1*lm12 + pwm2*lm22 + pwm3*lm32;
  outWorldMatrix[7] = pwm0*lm03 + pwm1*lm13 + pwm2*lm23 + pwm3*lm33;

  pwm0 = parentWorldMat[8]; pwm1 = parentWorldMat[9]; pwm2 = parentWorldMat[10]; pwm3 = parentWorldMat[11];
  outWorldMatrix[8] = pwm0*lm00 + pwm1*lm10 + pwm2*lm20 + pwm3*lm30;
  outWorldMatrix[9] = pwm0*lm01 + pwm1*lm11 + pwm2*lm21 + pwm3*lm31;
  outWorldMatrix[10] = pwm0*lm02 + pwm1*lm12 + pwm2*lm22 + pwm3*lm32;
  outWorldMatrix[11] = pwm0*lm03 + pwm1*lm13 + pwm2*lm23 + pwm3*lm33;

  pwm0 = parentWorldMat[12]; pwm1 = parentWorldMat[13]; pwm2 = parentWorldMat[14]; pwm3 = parentWorldMat[15];
  outWorldMatrix[12] = pwm0*lm00 + pwm1*lm10 + pwm2*lm20 + pwm3*lm30;
  outWorldMatrix[13] = pwm0*lm01 + pwm1*lm11 + pwm2*lm21 + pwm3*lm31;
  outWorldMatrix[14] = pwm0*lm02 + pwm1*lm12 + pwm2*lm22 + pwm3*lm32;
  outWorldMatrix[15] = pwm0*lm03 + pwm1*lm13 + pwm2*lm23 + pwm3*lm33;*/
...
/*
void _updatePosRotScale() {
if(_needsWorldUpdate == false) return;
_needsWorldUpdate = false;
_needsWorldMatrixUpdate = true;

if(gameObject.parent != null) {
var pTransform = gameObject.parent.transform;
pTransform._updatePosRotScale();
Vector3 pPos = pTransform._position;
Quaternion pRot = pTransform._rotation;
/*Vector3 pScale = pTransform._scale;
//pPos.copyInto(_position);

double newScaleX = _localScale.storage[0] * pScale.storage[0];
double newScaleY = _localScale.storage[1] * pScale.storage[1];
double newScaleZ = _localScale.storage[2] * pScale.storage[2];

_scale.storage[0] = newScaleX;
_scale.storage[1] = newScaleY;
_scale.storage[2] = newScaleZ;
 */

//pRot.rotate(_position);
_position.setFrom(_localPosition);
//_position.storage[0] = (newScaleX * _localPosition.storage[0]);
//_position.storage[1] = (newScaleY * _localPosition.storage[1]);
//_position.storage[2] = (newScaleZ * _localPosition.storage[2]);

pRot.rotate(_position);

_position.storage[0] += pPos.storage[0];
_position.storage[1] += pPos.storage[1];
_position.storage[2] += pPos.storage[2];

_rotate(_localRotation,pRot,_rotation);
} else {
//print('möööp? ${gameObject.name}');
_localPosition.copyInto(_position);
_localRotation.copyTo(_rotation);
//_localScale.copyInto(_scale);
}
}

void _updateWorldMatrix() {
setWorldMatrixFromPosRot(_position,_rotation,/*_scale,*/_worldTransform);
}*/
